...
params = [{'Kd': 1000, 'Bd':8.0, 'Md':0.016, 'wu':3},
...
if check_plot:
        plt.plot(curves[:, 0, 0], curves[:, 0, 2], 
                curves[:, 1, 0], curves[:, 1, 2], 
                curves[:, 2, 0], curves[:, 2, 2])
        plt.grid()
        plt.show(block=False)

if threeD_plot:
        # 3D plot with projections:
        ax = plt.figure().add_subplot(projection='3d', xticklabels=[], yticklabels=[], zticklabels=[])
        ...
        ax.plot(curves[:, 0, 0], curves[:, 0, 1], zs=z_off, zdir='z', color='royalblue',   linestyle='--', alpha=0.90)
        ax.plot(curves[:, 0, 0], curves[:, 0, 2], zs=y_off, zdir='y', color='palegreen',   linestyle='--', alpha=0.99)
        ax.plot(curves[:, 0, 1], curves[:, 0, 2], zs=x_off, zdir='x', color='forestgreen', linestyle='--', alpha=0.90)
...
#ax.set_proj_type('ortho')
#ax.plot(curves[:, 0, 0], curves[:, 0, 1], curves[:, 0, 2])
#ax.plot(curves[:, 1, 0], curves[:, 1, 1], curves[:, 1, 2])
#ax.plot(curves[:, 2, 0], curves[:, 2, 1], curves[:, 2, 2])
#plt.show(block=True)
