...
ts_color = '#2166ac'
js_color = '#2166ac'
font = {'family': 'serif', 'math_fontfamily': 'cm',
        'color': 'black', 'weight': 'semibold', 'size': 13}
...
curves = np.zeros((steps, len(params), 3)) # 1st dim: time, 2nd dim: params, 3rd dim: {x,y,z} 
...
plt.plot(rad2deg*jsdata_timp[t_offset:, 1], jsdata_timp[t_offset:, 7], color=js_color, linestyle='-.')
plt.ylabel('$\\tau_i$', fontdict=font)
plt.xlabel('$q_1$', fontdict=font)

plt.subplot(2, 9, 11, xticklabels=[], yticklabels=[])
plt.plot(rad2deg*jsdata_timp[t_offset:, 2], jsdata_timp[t_offset:, 8], color=js_color, linestyle='-.')
plt.xlabel('$q_2$', fontdict=font)

plt.subplot(2, 9, 12, xticklabels=[], yticklabels=[])
plt.plot(rad2deg*jsdata_timp[t_offset:, 3], jsdata_timp[t_offset:, 9], color=js_color, linestyle='-.')
...
plt.plot(rad2deg*jsdata_jimp[t_offset:, 1], jsdata_jimp[t_offset:, 7], color=js_color, linestyle='-.')
plt.xlabel('$q_1$', fontdict=font)

plt.subplot(2, 9, 14, xticklabels=[], yticklabels=[])
plt.plot(rad2deg*jsdata_jimp[t_offset:, 2], jsdata_jimp[t_offset:, 8], color=js_color, linestyle='-.')
plt.xlabel('$q_2$', fontdict=font)

plt.subplot(2, 9, 15, xticklabels=[], yticklabels=[])
plt.plot(rad2deg*jsdata_jimp[t_offset:, 3], jsdata_jimp[t_offset:, 9], color=js_color, linestyle='-.')
plt.xlabel('$q_3$', fontdict=font)
...
ax = plt.subplot(2, 9, 16, projection='3d', xticklabels=[], yticklabels=[], zticklabels=[])
ax.set_proj_type('ortho')
ax.plot(curves[:, 2, 0], curves[:, 2, 1], curves[:, 2, 2], color=blue1, label=f'Kd = {K[0]}')
ax.plot(curves[:, 1, 0], curves[:, 2, 1], curves[:, 1, 2], color=blue2, label=f'Kd = {K[1]}')
ax.plot(curves[:, 0, 0], curves[:, 2, 1], curves[:, 0, 2], color=blue3, label=f'Kd = {K[2]}')
...
plt.subplots_adjust(left=0.11, bottom=0.10, right=0.9, top=0.90, wspace=0.23, hspace=0.26)
plt.show(block=True)